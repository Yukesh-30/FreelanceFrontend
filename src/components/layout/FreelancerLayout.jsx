...
const SidebarIcon = ({ name, active }) => {
    switch (name) {
        ...
        case 'search':
            return <svg className={`w-5 h-5 ${active ? 'text-black' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>;
        case 'logout':
...
const FreelancerLayout = ({ children }) => {
    ...
    const navItems = [
        ...
        { name: 'Gig Orders', path: '/freelancer/orders', icon: 'settings' },
        { name: 'Messages', path: '/freelancer/chat', icon: 'chat' },
                            ...
                            return (
                                ...
                                >
                                    <span className={`mr-3 ${active ? 'text-white' : ''}`}>
                                        ...
                                        {item.icon === 'search' && <svg className={`w-5 h-5 ${active ? 'text-white' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>}
                                        ...
                                        {item.icon === 'chat' && <svg className={`w-5 h-5 ${active ? 'text-white' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>}
                                    </span>
                                    {item.name}
                                </Link>
                            );
